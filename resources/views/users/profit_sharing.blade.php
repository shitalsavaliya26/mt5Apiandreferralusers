...
@section('content')
    ...
    <nav class="nav flex-column flex-sm-row justify-content-sm-center reportbuttons" style="flex-wrap:unset;">
        ...
        <a class="mb-3 mb-xl-0 btn btn-primary btn-rounded"
    ...
    <header class="section-title titlebottomline">
        <h2 class="hrowheading">@lang('sidebar.profit_sharing')</h2>
    </header>
    <div class="card">
        <div class="card-body">
            <div class="d-block d-md-flex row justify-content-between mb-3">
                <div class="col grid-margin grid-margin-md-0">
                    ...
                    <form method="post" action="{{route('profit-sharing')}}">
                        @csrf
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                    @if($profitSharing)
			@php $x = $profitSharing->firstItem() @endphp
                        @foreach($profitSharing as $i => $sharing)
                            <tr>
                                <td>{{$x++}}</td>
                                <td>{{$sharing->profit}}</td>
                                <td id="breakdown_date">{{$sharing->created_at->format('m/d/Y')}}</td>
                                <td><a href="#" class="view_breakdown"
                                       data-date="{{$sharing->created_at}}">@lang('general.view')
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row ml-auto">
                                <input type="text" placeholder="@lang('general.search_username')" id="username"
                                       name="username"
            ...
            <div class="d-block d-md-flex row justify-content-between mt-4 align-items-center">
                ...
                @if($profitSharing->total() > 0)
                    <div class="col grid-margin grid-margin-md-0">@lang('general.showing') {{ $profitSharing->firstItem() }}
                        to {{ $profitSharing->lastItem() }} of {{ $profitSharing->total() }} @lang('general.entries')
                    </div>
                @else
                    ...
                    <div class="card-body">
                        <div class="row align-items-center">
                            <div class="col">
                                ...
                                <h3 class="mb-0 text-uppercase">IDR {{$totalProfitSharing ? $totalProfitSharing : '0'}}</h3>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('page_js')
    <script type="text/javascript">
        document.getElementById('pagination').onchange = function () {
            window.location = "{{ $profitSharing->url(1) }}&items=" + this.value;
        };

        $('.view_breakdown').on('click', function () {
            var breakdown_date = $(this).data('date');
            $.ajax({
                url: '{{ route('get-profit-sharing-breakdown') }}',
                data: {
                    date: breakdown_date
                },
                dataType: 'json',
        ...
        $("#search_username").click(function () {
            ...
            $.ajax({
                ...
                url: '{{route('search-profit-sharing-breakdown')}}',
                data: {
