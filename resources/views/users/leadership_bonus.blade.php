...
@section('content')
    ...
    <nav class="nav flex-column flex-sm-row justify-content-sm-center reportbuttons" style="flex-wrap:unset;">
        ...
        <a class="mb-3 mb-xl-0 btn btn-primary btn-rounded" href="{{route('leadership-bonus')}}">@lang('sidebar.leadership_bonus')</a>
        <a class="mb-3 mb-xl-0 btn btn-light btn-rounded" href="{{route('ownership-bonus')}}">@lang('sidebar.ownership_bonus')</a>
    ...
    <header class="section-title titlebottomline">
        <h2 class="hrowheading">@lang('sidebar.leadership_bonus')</h2>
    </header>
    <div class="card leadership-bonus">
        <div class="card-body">
            <div class="d-block d-md-flex row justify-content-between mb-3">
                <div class="col grid-margin grid-margin-md-0">
                    ...
                    <form method="post" action="{{route('leadership-bonus')}}">
                        @csrf
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                    @if($leadershipBonus)
			@php $x = $leadershipBonus->firstItem() @endphp
                        @foreach($leadershipBonus as $i => $bonus)
                            <tr>
                                <td>{{$x++}}</td>
                                <td>{{$bonus->profit}}</td>
                                <td id="breakdown_date">{{$bonus->created_at->format('m/d/Y')}}</td>
                                <td><a href="#" class="view_breakdown" data-date="{{$bonus->created_at}}">@lang('general.view')
            ...
            <div class="d-block d-md-flex row justify-content-between mt-4 align-items-center">
                ...
                @if($leadershipBonus->total() > 0)
                    <div class="col grid-margin grid-margin-md-0">@lang('general.showing') {{ $leadershipBonus->firstItem() }}
                        to {{ $leadershipBonus->lastItem() }} of {{ $leadershipBonus->total() }} @lang('general.entries')
                    </div>
                @else
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row ml-auto">
                        ...
                        <button type="submit" name="submit"
                                class="btn-outline-twitter input-group-text mx-1" id="search_username">@lang('general.search')
                        </button>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover">
                        ...
                        <div class="row align-items-center">
                            <div class="col">
                                <p class="card-title mb-2">
                                    @lang('general.total') @lang('general.amount')
                                </p>
                                <h3 class="mb-0 text-uppercase">IDR {{$totalLeadershipBonus ? $totalLeadershipBonus : '0'}}</h3>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('page_js')
    <script type="text/javascript">
        document.getElementById('pagination').onchange = function () {
            window.location = "{{ $leadershipBonus->url(1) }}&items=" + this.value;
        };

        $('.view_breakdown').on('click', function () {
            var breakdown_date = $(this).data('date');
            $.ajax({
                url: '{{ route('get-leadership-bonus-breakdown') }}',
                data: {
                    date: breakdown_date
                },
                dataType: 'json',
        ...
        $("#search_username").click(function () {
            ...
            $.ajax({
                ...
                url: '{{route('search-leadership-bonus-breakdown')}}',
                data: {
