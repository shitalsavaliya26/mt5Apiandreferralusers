...
@section('content')
        ...
        <div class="card-body">
            <div class="d-block csu-d-lg-flex row justify-content-between">
                <!-- <div class="grid-margin grid-margin-md-0 col">
                    <div class="">
                        <form method="post" action="{{route('news.all')}}">
                            @csrf
                            <div class="row">
                                <div style="" class="col-lg-2 col-sm-6 col-12 col-md-3 cus-m-mt-3">
                                ...
                                <div  class="col-lg-2 col-sm-6 col-12 col-md-3 cus-m-mt-3">
                                ...
                                <div  class="col-lg-2 col-sm-6 col-12 col-md-3 cus-m-mt-3 pl-0">
                                    <button type="submit" name="" id="search_username" class="form-control btn-reset">
                                        @lang('general.search')
                                    </button>
                                </div>
                                <div  class="col-lg-2 col-sm-6 col-12 col-md-3 cus-m-mt-3 pl-0">
                                    <a href="{{route('news.all')}}" id="" class="form-control btn-reset" style=" text-decoration: none;"onmouseover = "$(this).css('color','white');">
                                        @lang('general.reset')
                                    </a>
                                </div>
                            </div>
                        ...
                        <form>
                            <div class="d-flex flex-wrap align-items-center">
                                ...
                                <select id="pagination" class="form-control form-control-sm showperpage">
                                    <option value="9" @if($items == 9) selected @endif >9</option>
                                    <option value="18" @if($items == 18) selected @endif >18</option>
                                    <option value="36" @if($items == 36) selected @endif >36</option>
                                    <option value="72" @if($items == 72) selected @endif >72</option>
                    ...
                    <div class="card">
                        <a href="#" data-image="{{$n->image}}" data-title="{{$n->title}}" data-details="{{$n->details}}"
                           class="color-inherit text-decoration-none view_news">
                            ...
                            <img class="card-img-top" src="{{asset('news_images')}}/{{$n->image}}" style="height:150px;
                                 max-width:450px" alt="">
                            <div class="card-body card-cus-details" id="image-size">
                                <h4 class="card-title mt-3 boldbultertext">{{$n->title}}</h4>
                                <div class="card-text"><p>{!!  substr(strip_tags($n->details), 0, 100) !!}</p></div>
                            </div>
                        </a>
                    </div>
                </div>
    ...
    <div class="cardbg">
        <div class="d-block d-md-flex row justify-content-between align-items-center">
            ...
            @if($news->total() > 0)
                <div class="col grid-margin grid-margin-md-0">@lang('general.showing') {{ $news->firstItem() }}
                    to {{ $news->lastItem() }}
                    of {{ $news->total() }} @lang('general.entries')
                </div>
            @else
...
@section('page_js')
    <script>
        document.getElementById('pagination').onchange = function () {
            window.location = "{{ $news->url(1) }}&items=" + this.value;
        };
